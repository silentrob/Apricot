...
//
// Remark: There are issues around the jsdom integration. 0.0.1 is out of date.
//         tmpvar has pushed 0.1.2 but it's not @stable in npm. We need to 
//         get this resolved ASAP. 
// 
require.paths.unshift(require('path').join(__dirname, '../../jsdom/lib/'));
...
Apricot.parse = function (data, callback) {
  try {
    callback(null, new Apricot(data));
  }
  catch (error) {
    callback(error);
  }
